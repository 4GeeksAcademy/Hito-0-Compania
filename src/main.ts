// src/main.ts
import { type Envio } from "./types/models.js"; // <-- Corregido con 'type'
import {
  filtrarPorTransportista,
  ordenarPorCostoEnvio,
} from "./utils/collections.js";
import { busquedaBinariaEnvio } from "./utils/search.js";
import {
  contarEnviosDevueltos,
  calcularCostoTotalEnvios,
  calcularPromedioPeso,
  obtenerEnvioMasCostoso,
  obtenerEnvioMenosCostoso,
} from "./utils/transformations.js";
import { validarEnvio } from "./utils/validations.js";

const enviosPrueba: Envio[] = [
  {
    id: 1001,
...
function obtenerContenedorResultados(): HTMLDivElement {
  ...
  return elemento as HTMLDivElement;
}
...
function agregarTituloResultado(titulo: string): void {
  const contenedor = obtenerContenedorResultados();
...
function agregarTextoResultado(texto: string): void {
  const contenedor = obtenerContenedorResultados();
...
function agregarBloqueJsonResultado(data: unknown): void {
  const contenedor = obtenerContenedorResultados();
...
function obtenerListaDatosPrueba(): HTMLUListElement {
  ...
  return elemento as HTMLUListElement;
}

function crearTextoResumenEnvio(envio: Envio): string {
  return (
    "ID " +
    envio.id +
    " | Cliente: " +
...
function renderizarDatosDePrueba(): void {
...
function validarDatosIniciales(): string[] {
...
function obtenerEnviosOrdenadosPorId(): Envio[] {
...
function manejarClickFiltrarDinamico(): void {
  const select = document.getElementById("select-transportista") as HTMLSelectElement | null;
...
function manejarClickOrdenarPorCosto(): void {
...
function manejarClickCalcularReportes(): void {
...
function manejarClickBusquedaBinaria(): void {
  const input = document.getElementById("input-busqueda-id") as HTMLInputElement | null;
